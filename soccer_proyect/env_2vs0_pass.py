...
class stage_soccerTraining_pass(wrap.DmGoalWrapper):
	...
	def get_Area(self, obs):
		...
		for team_pos, team_or in zip(teammate_pos, teammate_orientation):
			...
			for angle_opp, pos in zip(opp_orientation, opp_pos):
				area = self.calculate_pass_cone_intersection(r_mid, theta_mid, team_or, angle_opp, pos[0], pos[1], pass_=True, team_pos = team_pos)
				pass_cone_area += area
	...
	def get_mid_cone(self, team_pos, angle_ego):
