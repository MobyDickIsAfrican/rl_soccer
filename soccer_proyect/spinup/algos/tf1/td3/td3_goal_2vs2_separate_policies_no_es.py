...
def td3(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=None, 
        steps_per_epoch=10000, epochs=10000, replay_size=int(1e7), gamma=0.99, 
    ...
    if gradient_clipping:
        grads_and_vars_pi_1  = pi_optimizer_1.compute_gradients(pi_loss_1, var_list=(get_vars('main/player_1/pi')))
        grads_pi_1 = [g for g, v in grads_and_vars_pi_1]
        vars_pi_1 = [v for g, v in grads_and_vars_pi_1]
        capped_grads_pi_1, __ = tf.clip_by_global_norm(grads_pi_1, 1.0)
        train_pi_op_1 = pi_optimizer_1.apply_gradients(zip(capped_grads_pi_1, vars_pi_1))
        
        grads_and_vars_pi_2  = pi_optimizer_2.compute_gradients(pi_loss_2, var_list=(get_vars('main/player_2/pi')))
        grads_pi_2 = [g for g, v in grads_and_vars_pi_2]
        vars_pi_2 = [v for g, v in grads_and_vars_pi_2]
        capped_grads_pi_2, __ = tf.clip_by_global_norm(grads_pi_2, 1.0)
        train_pi_op_2 = pi_optimizer_2.apply_gradients(zip(capped_grads_pi_2, vars_pi_2))

        grads_and_vars_q_1 = q_optimizer_1.compute_gradients(q_loss_1, var_list=(get_vars('main/player_1/q')))
        grads_q_1 = [g for g, v in grads_and_vars_q_1]
        vars_q_1 = [v for g, v in grads_and_vars_q_1]
        capped_grads_q_1, __ = tf.clip_by_global_norm(grads_q_1, 1.0)
        train_q_op_1 = q_optimizer_1.apply_gradients(zip(capped_grads_q_1, vars_q_1))
        
        grads_and_vars_q_2 = q_optimizer_2.compute_gradients(q_loss_2, var_list=(get_vars('main/player_2/q')))
        grads_q_2 = [g for g, v in grads_and_vars_q_2]
        vars_q_2 = [v for g, v in grads_and_vars_q_2]
        capped_grads_q_2, __ = tf.clip_by_global_norm(grads_q_2, 1.0)
        train_q_op_2 = q_optimizer_2.apply_gradients(zip(capped_grads_q_2, vars_q_2))
    ...
    if aux_int:
        ...
        if isinstance(load_1vs1, str):
            assert isinstance(num, int)
            
            with g1.as_default():
                __, _get_1v1_action_p1 = load_policy_and_env(load_1vs1, num, sess=None)
                
            _get_1v1_action_p2 = lambda x: _get_1v1_action_p1(x)
        
        elif isinstance(load_1vs1, list) or isinstance(load_1vs1, tuple):
            assert len(load_1vs1) == 2
            assert (isinstance(num, list) or isinstance(num, tuple)) and 2 == len(num)
            
            with g1.as_default():
                __, _get_1v1_action_p1 = load_policy_and_env(load_1vs1[0], num[0], sess=None)
            
            g2 = tf.Graph()
            with g2.as_default():
                __, _get_1v1_action_p2 = load_policy_and_env(load_1vs1[1], num[1], sess=None)
        
        else:
            raise ValueError(f"Invalid value for load_1vs1, expected str, tuple or list, got: {load_1vs1}")
        ...
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                if aux_int:
                    ...
                    act_1 = []
                a = act_1 + get_action(np.array(o[aux_int:]), 0, (num_players - aux_int) // 2)
                ...
                for k in range(num_players):
                    ...
                    _switch_k = (aux_int - k - 1) if ((k < aux_int) and (j % 2 == 1)) else k
                ...
                ep_len += 1
            success_rate += (aux_int > 0) * (ep_len <= max_ep_len and test_env.timestep.reward[0] < 0) / num_test_episodes + \
                            (1 - (aux_int > 0)) * (ep_len <= max_ep_len) / num_test_episodes
    ...
    for t in range(total_steps):
        ...
        if switch:
            act_1 = get_1v1_action_1(o[0][np.r_[0:18, 18:24]]) + get_1v1_action_2(o[1][np.r_[0:18, 24:30]]) if aux_int else []
        else:
            act_1 = get_1v1_action_2(o[0][np.r_[0:18, 18:24]]) + get_1v1_action_1(o[1][np.r_[0:18, 24:30]]) if aux_int else []

        if t > start_steps:
            a = act_1 + get_action(np.array(o[aux_int:]), act_noise, (num_players - aux_int) // 2)
        else:
            a = act_1 + [env.action_space.sample() for _ in range(aux_int, num_players)]
...
if __name__ == '__main__':
    ...
    td3(lambda : dm_soccer2gym.make('2vs2goal', task_kwargs={"rew_type": args.reward, "time_limit": args.time_limit, "disable_jump": True, 
        ...
        load_1vs1=["/home/amtc/pavan/rl_soccer/models/TD3/paper/1vs1/2020-10-08_23-06-32_td3_soccer_goal_1vs1_simple_v2_0.05",
                   "/home/amtc/pavan/rl_soccer/models/TD3/paper/1vs1/2020-10-08_23-07-33_td3_soccer_goal_1vs1_simple_v2_0.05"],
        num=[9389999, 8629999])
