...
def main(agent_1, agent_2, sess=None, order='home_away'):
    ...
    if order == 'home_away':
        ...
        _, get_action_2 = load_policy_and_env(*agent_1, sess=sess)
            
        g2 = tf.Graph()
        
        with g2.as_default():
            _, get_action_1 = load_policy_and_env(*agent_2, sess=None)
    ...
    for j in range(num_runs // 2):
        ...
        while not(d or (l == max_ep_len)):
            a = [get_action_1(obs[0][:18]), get_action_2(obs[1][:18])]
            if RENDER:
                env.render()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agent_1_idx", type=int, help="Agent 1 index")
    parser.add_argument("--agent_2_idx", type=int, help="Agent 2 index")
    ...
    s, t, f = main(agents[args.agent_1_idx], agents[args.agent_2_idx], sess, args.order)
    
    os.chdir(orig_path)
    os.makedirs(args.save_path, exist_ok=True)
        
    with open(os.path.join(args.save_path, f'{args.agent_1_idx}vs{args.agent_2_idx}.txt'), 'a') as f_:
