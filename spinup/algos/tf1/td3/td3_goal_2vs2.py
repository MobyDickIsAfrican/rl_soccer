...
def td3(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
        ...
        logger_kwargs=dict(), save_freq=1, sess=None, load_1vs1="", gradient_clipping=False):
    """
    ...
    if aux_int:
        g = tf.Graph()
        with g.as_default():
            __, _get_1v1_action = load_policy_and_env(load_1vs1, sess=None)
            get_1v1_action_1 = lambda x: [_get_1v1_action(x[np.r_[0:18, 18:24]])]
            get_1v1_action_2 = lambda x: [_get_1v1_action(x[np.r_[0:18, 24:30]])]
    ...
    def test_agent():
        ...
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                act_1 = get_1v1_action_1(o[0]) + get_1v1_action_2(o[1]) if aux_int else []
                ...
                if j == 0:
                    test_env.render()
            success_rate += (aux_int > 0) * (ep_len < max_ep_len and r[aux_int] > 0) / num_test_episodes + (1 - (aux_int > 0)) * (ep_len < max_ep_len) / num_test_episodes
    ...
    for t in range(total_steps):
        ...
        act_1 = get_1v1_action_1(o[0]) + get_1v1_action_2(o[1]) if aux_int else []
        if t > start_steps:
            a = act_1 + get_action(np.array(o[aux_int:]), act_noise, num_players - aux_int)
        else:
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            if ((epoch % save_freq == 0) or (epoch == epochs)) and (act_suc_rate > 0.75):
...
if __name__ == '__main__':
    ...
    logger_kwargs = setup_logger_kwargs('td3_soccer_goal_2vs2', data_dir="/home/pavan/Proyecto_EL7021/models/TD3", datestamp=True)
    gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=args.gpu)

    sess = tf.Session(config=tf.ConfigProto(gpu_options=gpu_options))
    
    td3(lambda : dm_soccer2gym.make('2vs2goal', task_kwargs={"rew_type": "sparse", "time_limit": 45., "disable_jump": True, 
        ...
        load_1vs1="/home/pavan/Proyecto_EL7021/models/TD3/2020-06-30_td3_soccer_goal_1vs1_heads_sparse")
